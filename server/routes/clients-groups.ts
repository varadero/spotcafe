...
export class ClientsGroupsRoutes extends RoutesBase {
    ...
    getAllClientsGroups(): any {
        return route.get(this.apiPrefix + 'clients-groups', async ctx => {
            await this.handleResult(ctx, () => this.storageProvider.getClientsGroups());
        });
    }

    updateClientGroup(): any {
        return route.post(this.apiPrefix + 'clients-groups/:id', async ctx => {
            await this.handleActionResult(ctx, () => this.updateClientGroupImp(ctx.request.body));
        });
    }

    createClientGroup(): any {
        return route.post(this.apiPrefix + 'clients-groups', async ctx => {
    ...
    private async updateClientGroupImp(
        ...
        const updateResult = await this.storageProvider.updateClientGroup(clientGroup);
    ...
    private async createClientGroupImp(
        ...
        const createReslt = await this.storageProvider.createClientGroup(clientGroup);
        return { value: createReslt };
    }

    private sanitizeClientGroup(clientGroup: IClientGroup): IRouteActionResult<void> | null {
